...
from ..adapter_utils import Adapter
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if self.with_adapter:
            self.adapter = Adapter(in_channels=channels,
    ...
    def _forward(self, x, emb):
        if self.updown:
            ...
            x = self.x_upd(x)
            if self.with_adapter:
                adapter_out = self.adapter(x, emb)
            else:
                adapter_out = 0.0
            h = in_conv(h)
        else:
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
        return self.skip_connection(x) + h + adapter_out
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        h = self.proj_out(h)
        return (x + h).reshape(b, c, *spatial) + adapter_out
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        return scaled_dot_product_attention(q, k, v).reshape(bs, -1, length)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.with_adapter:
            self.out_adapter = Adapter(in_channels=input_ch,
                                       out_channels=self.out_channels if self.out_channels != 6 else 3,
                                       temb_channels=None,
                                       embed_dim=adapter_dim,
                                       patch_size=adapter_patch_size,
                                       num_heads=adapter_num_heads,
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # end
        if self.with_adapter:
            adapter_out = self.out_adapter(h, None)
        else:
            adapter_out = 0.0
        return self.out(h)[:, :self.out_channels if self.out_channels != 6 else 3, :] + adapter_out
